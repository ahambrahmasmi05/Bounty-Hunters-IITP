...
define( 'MWAI_FORMS_SERVER_PARAMS', [ 'model', 'temperature', 'maxTokens', 'prompt', 'envId', 'env', 'resolution',
...
class MeowPro_MWAI_Forms {
  ...
  function rest_submit( $request ) {
    try {
      ...
      $model = $systemParams['model'];

      // Prepare the message (based on the fields).
      $message = isset( $params['prompt'] ) ? $params['prompt'] : $systemParams['prompt'] ?? "";
